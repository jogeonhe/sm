...
gsap.to(about, {
  ...
  scrollTrigger: {
    ...
    toggleActions: 'play none play none', // Adjust toggleActions to ensure smooth transition
    onEnter: () => {
      gsap.to(about, { opacity: 1, duration: 0.5 }); // Smooth transition to opacity 1
    },
    onLeave: () => {
      gsap.to(about, { opacity: 0, duration: 0.5 }); // Smooth transition to opacity 0
    },
    onEnterBack: () => {
      gsap.to(about, { opacity: 1, duration: 0.5 }); // Smooth transition to opacity 1 when scrolling back
    },
    onLeaveBack: () => {
      gsap.to(about, { opacity: 0, duration: 0.5 }); // Smooth transition to opacity 0 when leaving again
    }
  }
});

// for aboutText element
gsap.to(aboutText, {
  x: -100,
  opacity: 1,
  duration: 1,
  scrollTrigger: {
    ...
    toggleActions: 'play none play none', // Adjust toggleActions to ensure smooth transition
    onEnter: () => {
      gsap.to(aboutText, { opacity: 1, duration: 0.2 }); // Smooth transition to opacity 1
    },
    onLeave: () => {
      gsap.to(aboutText, { opacity: 0, duration: 0.2 }); // Smooth transition to opacity 0
    },
    onEnterBack: () => {
      gsap.to(aboutText, { opacity: 1, duration: 0.2 }); // Smooth transition to opacity 1 when scrolling back
    },
    onLeaveBack: () => {
      gsap.to(aboutText, { opacity: 0, duration: 0.2 }); // Smooth transition to opacity 0 when leaving again
    }
  }
});
