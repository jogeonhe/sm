...
function kpop() {
  ...
  //     scrub:false
  //   })
  //   const timelineMap = gsap.timeline({
  //     scrollTrigger: {
  //       trigger: mapWrap,  // mapWrap을 트리거로 설정
  //       start: 'top top',   // 애니메이션 시작 시점
  //       end: 'top top',     // 종료 시점 (여기서는 고정)
  //       pin: true,          // mapWrap 고정
  //       pinSpacing: false,  // 고정 후 공간 확보를 막음
  //       scrub: 0,           // 애니메이션 동기화 (즉시 진행)
  //       markers: false
  //     }
  //   });
    
  //   timelineMap.to(mapWrap, {
  //     y: -500,    // mapWrap을 위로 이동 (위치 조정)
  ...
  //     duration:1
  //   },"<0.7")
  //   .to([firstCircleFront,secondCircleFront,thirdCircleFront,lastCircleFront,midCircle],{
  //     opacity:1,
  //     duration:1,
...
function about() {
  ...
  const animationAbout = gsap.timeline({
    scrollTrigger: {
      trigger: contentWrap,  // 동일한 트리거
      start: 'top top',       // 시작 시점
      end: '200% top',        // 종료 시점 (애니메이션 범위를 다르게 설정)
      scrub: 3,               // 애니메이션 스크럽 속도
      pin: true,              // 섹션 고정
      pinSpacing: true,
      markers: false
    }
