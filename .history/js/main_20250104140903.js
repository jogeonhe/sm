document.addEventListener('DOMContentLoaded', () => {
  ...
  gsap.to(about, {
    ...
    scrollTrigger: {
      trigger: aboutWrap,
    start: 'top 80%', // aboutWrap이 화면 상단에서 80%에 도달할 때 애니메이션 시작
    end: 'bottom top', // aboutWrap이 화면 아래에서 top에 도달할 때 애니메이션 종료
    markers: true,  // 애니메이션 트리거 위치 표시
    toggleActions: 'play none none none',  // 애니메이션을 한 번만 실행되도록 설정
    once: true, // 스크롤을 올려도 애니메이션이 다시 실행되지 않도록 설정
    }
  })
  gsap.to(aboutText,{x:-100,opacity:1,duration:3,scrollTrigger:{
    trigger:aboutWrap,
    start:'90% 0'
  }})
