...
lines.forEach((line, index) => {
  ...
  gsap.to(line, {
      ...
      duration: 1,  // 선 그려지는 시간
      delay: index * 2, // 선들이 2초 간격으로 그려지도록 설정
      opacity: 1,
      scrollTrigger: {
          trigger: line,  // 트리거될 요소
          start: "'900% 0", // 스크롤이 해당 요소에 80% 도달할 때 시작
          end: "900% 0",   // 해당 요소가 끝날 때까지 애니메이션 진행
          once: true, // 한 번만 실행
      }
  });
});
...
circles.forEach((circle, index) => {
  ...
  gsap.to(circle, {
      ...
      duration: 1,  // 원 그려지는 시간
      delay: (index * 2) + 1, // 원들은 선 그려지고 1초 후에 그려지도록 설정
      opacity: 1,
      scrollTrigger: {
          trigger: circle, // 트리거될 요소
          start: "'80% 0", // 스크롤이 해당 요소에 80% 도달할 때 시작
          end: "900% 0",   // 해당 요소가 끝날 때까지 애니메이션 진행
          once: true, // 한 번만 실행
      }
  });
  ...
  gsap.to(texts[index], {
      opacity: 1,     // 텍스트 나타나게
      y: 0,           // 원위치로
      duration: 1,    // 텍스트가 나타나는 시간
      delay: (index * 2) + 2, // 원과 동시에 텍스트가 나옵니다
      scrollTrigger: {
          trigger: texts[index], // 트리거될 요소
          start: "'80% 0", // 스크롤이 해당 요소에 80% 도달할 때 시작
          end: "900% 0",   // 해당 요소가 끝날 때까지 애니메이션 진행
          once: true, // 한 번만 실행
      }
...
gsap.to(whiteCircle, {
  ...
  scrollTrigger: {
      trigger: whiteCircle,
      start: "'80% 0",
      end: "900% 0",  // 스크롤이 해당 요소에 80% 도달할 때 시작
      once: true, // 한 번만 실행
  }
});
...
gsap.to(lastLine, {
  ...
  scrollTrigger: {
      trigger: lastLine,
      start: "'900% 0",
      end: "buttom 900%",  // 스크롤이 해당 요소에 80% 도달할 때 시작
      once: true, // 한 번만 실행
  }
});
