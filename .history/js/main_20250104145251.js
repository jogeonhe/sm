document.addEventListener('DOMContentLoaded', () => {
  ...
  function kpop() {
    ...
    const globalWrap=document.querySelector('#global_wrap')
    /*책임*/ 
...
// 마지막 선 그려지는 애니메이션
    ...
    //   duration:30
    // })
    animation.to(globalWrap,{
      y:0,
      duration:30,
      delay: 35
    })
    gsap.to(globalTitle,{x:107,opacity:1,duration:3,scrollTrigger:{
      trigger: globalWrap,
      start: '80% 0', // 애니메이션 시작 지점
      end: '700% 0', // 애니메이션 종료 지점
      toggleActions: 'play none reverse none', // 스크롤 내리면 실행, 올리면 반대로 실행
      onEnter: () => {
        gsap.to(globalTitle, { opacity: 1 }); // 'aboutText'가 보일 때 opacity를 1로 설정
      },
      onLeave: () => {
        gsap.to(globalTitle, { opacity: 0 }); // 'aboutText'가 안 보일 때 opacity를 0으로 설정
      },
      onEnterBack: () => {
        gsap.to(globalTitle, { opacity: 1 }); // 다시 돌아올 때 'aboutText'을 보이게 설정
      },
      onLeaveBack: () => {
        gsap.to(globalTitle, { opacity: 0 }); // 다시 떠날 때 'aboutText'을 숨김
      }
    }})
