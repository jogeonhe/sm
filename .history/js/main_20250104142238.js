document.addEventListener('DOMContentLoaded', () => {
  ...
  gsap.to(about, {
    ...
    scrollTrigger: {
      trigger: aboutWrap,
      start: '90% 0',   // 스크롤이 aboutWrap의 90% 지점에 도달할 때 시작
      end: 'bottom top', // aboutWrap이 화면 하단에 도달하면 애니메이션 종료
      scrub: true,       // 스크롤에 따라 애니메이션이 진행됨
      toggleActions: 'play reverse play reverse', // 스크롤 내리면 실행, 올리면 반대로 실행
      markers: true      // 마커를 표시하여 트리거 범위 확인
    }
  });
  ...
  gsap.to(aboutText, {
    ...
    scrollTrigger: {
      trigger: aboutWrap,
      start: '90% 0',   // aboutText 애니메이션 시작 위치
      end: 'bottom top', // 애니메이션 종료 위치
      scrub: true,       // 스크롤에 맞춰 진행
      toggleActions: 'play reverse play reverse', // 스크롤 내리면 실행, 올리면 반대로 실행
      markers: true      // 마커 표시
    }
  });
