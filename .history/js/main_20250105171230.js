...
gsap.to(aboutText, {
  ...
  scrollTrigger: {
    ...
    onEnterBack: () => {
      gsap.to(aboutText, { opacity: 1, duration: 0.5 }); // Smooth transition to opacity 1 when scrolling back
    },
    onLeaveBack: () => {
      gsap.to(aboutText, { opacity: 0, duration: 0.5 }); // Smooth transition to opacity 0 when leaving again
    }
  }
});
