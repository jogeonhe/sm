...
function kpop() {
  ...
  animation
    ...
    // 선택된 경로들 콘솔에 출력하여 확인
    console.log(firstMapLinePath, secondMapLinePath, thirdMapLinePath, lastMapLinePath);  // 경로가 잘 선택되었는지 확인
    
    // 경로의 길이가 제대로 계산되는지 확인
    console.log(firstMapLinePath.getTotalLength());  // 길이가 제대로 계산되는지 확인
    ...
    timeline1
      ...
      .to([firstCircleBack, secondCircleBack, thridCircleBack, lastCircleBack],{
        opacity: 0.4,         // 오퍼시티를 1로 설정하여 원이 보이게 함
        duration: 0.7,        // 애니메이션 시간
        repeat: -1,         // 애니메이션을 무한 반복
        yoyo: true,         // 반복 시, 애니메이션이 반대로 진행되도록 설정 (없어졌다가 다시 나타남)
        ease: "power1.inOut", // 부드러운 easing 효과
        repeatDelay: 0.3,   // 반복 간의 딜레이 시간
      })
